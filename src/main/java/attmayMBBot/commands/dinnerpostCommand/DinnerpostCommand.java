...
public class DinnerpostCommand implements ICommand {
    ...
    public void execute(Message message, String[] args) {
        if(new AdvancedBotUserAuthorization(this.config).checkIfUserIsAuthorized(message.getAuthor().get())){
            Recipe recipe = new SpoonacularAPIHandler(this.config).getRandomRecipe();
            ...
            message.getChannel().block().createMessage("Well, I know this is somewhat awkward but you are not allowed to perform this command.");
    }
}
